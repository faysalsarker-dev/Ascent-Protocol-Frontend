...
export const ProgressRing = ({
  ...
  useAnimationFrame((t) => {
    time.set(t / 1000); // convert ms to seconds
  });
  ...
  return (
    <div className="relative" style={{ width: size + 40, height: size + 40 }}>
      ...
      {[...Array(6)].map((_, i) => {
        // Calculate X/Y transforms for each particle using useTransform
        const x = useTransform(time, (t) => Math.cos((i * Math.PI * 2) / 6 + t) * (size / 2 + 10));
        const y = useTransform(time, (t) => Math.sin((i * Math.PI * 2) / 6 + t) * (size / 2 + 10));
            ...
            <>
              ...
              >
                <span className="text-4xl font-display font-bold">
                  <GlitchText className="bg-linear-to-b from-foreground to-foreground/60 bg-clip-text text-transparent">
                    {`${Math.round(progress)}%`}
                  </GlitchText>
                </span>
              </motion.div>
